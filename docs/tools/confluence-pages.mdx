...
| `page_width` | `string` | No | (Optional) Page layout width. One of `full-width`, `max`, or `default`. When omitted, the Confluence space default is used. |

**Example:**

```json
{"space_key": "DEV", "title": "Architecture Decision Record", "content": "## Context\n\nWe need to decide...", "parent_id": "98765432", "page_width": "full-width"}
```

<Tip>
Use Markdown for content — it's auto-converted to Confluence storage format. Specify `parent_id` to create as a child page. Use `page_width: "full-width"` for wide tables or diagrams.
</Tip>
...
| `emoji` | `string` | No | (Optional) Page title emoji (icon shown in navigation). Can be any emoji character like '📝', '🚀', '📚'. Set to null/None to remove. |
| `page_width` | `string` | No | (Optional) Page layout width. One of `full-width`, `max`, or `default`. When omitted, the current width is preserved. |

**Example:**

```json
{"page_id": "12345678", "title": "Updated Title", "content": "## Updated Content\n\nNew information...", "is_minor_edit": true, "page_width": "full-width"}
```

<Tip>
Set `is_minor_edit: true` to skip notification emails. The page version is auto-incremented. `page_width` is persisted via Confluence content properties (`content-appearance-published` and `content-appearance-draft`); leaving it `null` keeps whatever width the page currently has.
</Tip>
